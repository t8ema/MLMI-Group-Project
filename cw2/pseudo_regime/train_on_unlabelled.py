...
output_dir = "MLMI-Group-Project/cw2/pseudo-regime/pseudo_train_data"
...
if re_process_data:
    ...
    num_unlabelled_images = 200  # Number of pseudo-labelled images to use
...
for step in range(total_iter):
    ...
    if step1 % freq_save == 0:
        model_save_path = os.path.join(SAVE_PATH, f"semi-supervised_unet_step_{step1}.tf")
        tf.saved_model.save(residual_unet_model, model_save_path)
        print(f"Model saved to {model_save_path}")

# Save final model
model_save_path = os.path.join(SAVE_PATH, f"semi-supervised_unet_step_finalstep.tf")
tf.saved_model.save(residual_unet_model, model_save_path)
print(f"Model saved to {model_save_path}")
