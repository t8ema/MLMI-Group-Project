# Makes predictions of pseudo_labels by ensemble, and then trains on a combination of the labelled and pseudo-labelled data
...
path_to_test = './MLMI-Group-Project/cw2/test'  # Directory containing test data

output_dir = "MLMI-Group-Project/cw2/SSL-pseudo/pseudo_train_data"
...
def re_process_data():
    ...
    # Delete files in pseudo-label training directory
    ...
    # Function to copy a specific number of valid files with naming convention
    def copy_files(source_dir, dest_dir, prefix, start_index, num_files):
        copied_count = 0  # To track how many valid files have been copied

        i = 0  # Index to iterate through potential files
        while copied_count < num_files:
            ...
            if os.path.exists(image_src) and os.path.exists(label_src):
                image_dest = os.path.join(dest_dir, f"image_pseudo{start_index + copied_count:03d}.npy")
                label_dest = os.path.join(dest_dir, f"label_pseudo{start_index + copied_count:03d}.npy")

                # Copy files
                shutil.copy(image_src, image_dest)
                shutil.copy(label_src, label_dest)
                print(f"Copied {image_src} and {label_src} to {image_dest} and {label_dest}")

                copied_count += 1  # Increment count only for valid pairs
            else:
                print(f"Warning: File {image_src} or {label_src} does not exist and will be skipped.")

            i += 1  # Move to the next potential file
...
class DataReader:
    ...
    def load_images_train(self, indices_mb):
        return self.load_npy_files(["image_pseudo%03d.npy" % idx for idx in indices_mb])
    def load_images_test(self, indices_mb):
        return self.load_npy_files(["image_pseudo%03d.npy" % idx for idx in indices_mb])
    def load_labels_train(self, indices_mb):
        return self.load_npy_files(["label_pseudo%03d.npy" % idx for idx in indices_mb])
