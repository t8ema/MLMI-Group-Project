# Processes data for pseudo-labelling experiment
...
test_dir = "MLMI-Group-Project/cw2/test"  # Directory for processed outputs
...
for dir_path in [train_dir, val_dir, test_dir, unlabel_dir]:
...
for img_file_name, mask_file_name in image_mask_pairs:
    ...
    if total_processed < num_train:
        ...
        np.save(img_output_path, resampled_img)
        np.save(mask_output_path, binary_mask)
    elif total_processed < num_train + num_val + num_test:
        img_output_path = os.path.join(test_dir, f"image_test{test_count:03d}.npy")
        mask_output_path = os.path.join(test_dir, f"label_test{test_count:03d}.npy")
        test_count += 1

        np.save(img_output_path, resampled_img)
        np.save(mask_output_path, binary_mask)
    elif total_processed < num_train + num_val + num_test + num_unlabel:
