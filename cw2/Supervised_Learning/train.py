# Train for supervised experiment
...
total_iter = 5000  # Total number of iterations
...
patience = 48  # Number of test intervals to wait for improvement
size_minibatch = 6
...
# Track dice values and losses for plotting later
test_dices = []
train_losses = []
step_number_counter = []
...
for step in range(total_iter):
    ...
    if step1 % freq_test == 0:
        ...
        step_number_counter.append(step1)
...
test_dices_path = os.path.join(SAVE_PATH, "test_dices.npy")
step_number_counter_path = os.path.join(SAVE_PATH, "step_number_counter.npy")

# Save arrays as NumPy files
np.save(test_dices_path, np.array(test_dices))
np.save(step_number_counter_path, np.array(step_number_counter))

print(f"Test dices saved to {test_dices_path}")
print(f"Step number counter saved to {step_number_counter_path}")
